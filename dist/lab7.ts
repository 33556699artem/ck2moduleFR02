export namespace Transport {
    ...
    export class CarCl extends VehicleClass implements Car {
        ...
        getInfoAboutBodyTypeAndCarClass(): {bodyType: BodyType, carClass: CarClass} {
            return {
    ...
    export class VehicleStorageCl<T extends Vehicle> implements VehicleStorage<T> {
        ...
        getByMark(mark: string): T[] {
            return this.vehicles.filter(v => v.getModel == mark);
        }
    }
}
...
const sorted = storage.sortByModel();
console.log(sorted);

const car3 = new Transport.CarCl("CADILLAC", "XT5", 2021, "0923FH028HFS0JBIF3347", "И123ВТ", owner1, Transport.BodyType.crossover, Transport.CarClass.Buisiness);
storage.addVehicle(car3);
const gbm = storage.getByMark("XT5");
console.log(gbm);

const info = car3.getInfoAboutBodyTypeAndCarClass();
console.log(info);